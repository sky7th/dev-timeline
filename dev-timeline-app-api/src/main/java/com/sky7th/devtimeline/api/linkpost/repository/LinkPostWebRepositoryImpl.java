...
import static com.sky7th.devtimeline.generalpost.linkpost.domain.QLinkPost.linkPost;
...
public class LinkPostWebRepositoryImpl implements LinkPostWebRepositoryCustom {
    ...
    @Override
    public List<LinkPostItem> findAllWithLikeCountAndIsLikeBySearchForm(
        LinkPostSearchForm postSearchForm) {
        return queryFactory
            .select(
                Projections.fields(LinkPostItem.class, linkPost,
                    ExpressionUtils.as(getLikedExpression(), "isLike")
...
//    }
    ...
    @Override
    public long countBySearchForm(LinkPostSearchForm postSearchForm) {
        return queryFactory
            .selectFrom(linkPost)
            .leftJoin(linkPost.post).fetchJoin()
            .where(containsTags(postSearchForm.getTags()),
    ...
    private BooleanBuilder containsTags(List<String> tags) {
        ...
        for (String tag : tags) {
            builder.or(linkPost.title.contains(tag));
        }
        return builder;
    }
